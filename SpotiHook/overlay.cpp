...
void initD3D(HWND hWnd)
{
	...
	D3DXCreateFontA(d3ddev, 12, 0, FW_MEDIUM, 1, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "icomoon", &pFont);
...
void DrawLine2(float x1, float y1, float x2, float y2, float size, bool antiAlias, zgui::color COLOR)
{
	D3DXVECTOR2 dLine[2];
	...
	d3dLine->Begin();
	d3dLine->Draw(dLine, 2, D3DCOLOR_RGBA(COLOR.r, COLOR.g, COLOR.b, 255));
	d3dLine->End();
}
...
void String(int x, int y, zgui::color colour, ID3DXFont* font, bool outlined, const char* string, ...)
{
	va_list args;
	char cBuffer[256];
	...
	pRect.left = x;
	pRect.top = y;
	font->DrawTextA(NULL, cBuffer, strlen(cBuffer), &pRect, DT_NOCLIP, D3DCOLOR_RGBA(colour.r, colour.g, colour.b, colour.a));
}

void FilledBox(int x, int y, int width, int height, zgui::color colour)
{
	SD3DVertex pVertex[4] = { { x, y + height, 0.0f, 1.0f, D3DCOLOR_RGBA(colour.r, colour.g, colour.b, 255) }, { x, y, 0.0f, 1.0f, D3DCOLOR_RGBA(colour.r, colour.g, colour.b, 255) }, { x + width, y + height, 0.0f, 1.0f, D3DCOLOR_RGBA(colour.r, colour.g, colour.b, 255) }, { x + width, y, 0.0f, 1.0f, D3DCOLOR_RGBA(colour.r, colour.g, colour.b, 255) } };
	d3ddev->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
	d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, pVertex, sizeof(SD3DVertex));
}


void BorderedBox(int x, int y, int width, int height, int thickness, zgui::color colour) {
